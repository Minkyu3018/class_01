...
-- 사원번호, 사원이름, 급여 3개의 칼럼으로 구성 된 EMP01 테이블을 생성해 봅시다. 
...
-- 서브쿼리를 이용해서 기존 테이블의 구조로 테이블을 생성, 테이터도 저장,
-- 제약조건은 복사되지 않는다.
...
-- 컬럼의 변경 : 테이블에 컬럼을 추가, 테이블에 있는 컬럼을 삭제, 컬럼의 내용이나 이름을
--           수정하는 것

-- 테이블의 변경
-- alter table
-- add(추가), modify(수정), drop(삭제)  [add, modify, drop] ()
...
-- 직급(JOB) 칼럼을 최대 20글자까지 저장할 수 있게 변경해 보도록 하자.
desc emp01;
...
-- unique : 컬럼 내부에 중복되는 데이터가 있으면 입력 실패, 수정실패
...
insert into emp01 values (1112, 'KING2', 'FREE', 1500, 'F', 40, sysdate);
